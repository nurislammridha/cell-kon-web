...
const UserAddressPage = ({ isLogin }) => {
    ...
    return (<>
        ...
        <div className='user_info'>
            <UserMenu buyerDetails={buyerDetails} activeTab='address' />
            <div className='right'>
                    ...
                    <div className='address_list'>
                        <div className='add'>
                            ...
                            <a href='/add-address'
                                ...
                                onClick={(event) => {
                                    ...
                                    navigate('/add-address', { state: { isFromCheckout: location?.state?.isFromChackout, selected: location?.state?.selected } })
                                }}
                            >Add Address</a>
                        </div>
                        {addressInfo?.length > 0 && addressInfo.map((item, index) => (
                            ...
                            >
                                <div className='title'>
                                    <p>{item?.buyerName}</p>
                                    {!location?.state?.isFromChackout && <a href={`/edit-address/${item._id}`} onClick={(event) => {
                                        event.preventDefault()
